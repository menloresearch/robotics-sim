...
public:
  ...
  val mesh_texcoord() const {
    return val(typed_memory_view(m->nskintexvert * 2, m->mesh_texcoord));
  }
  val mesh_face() const {
  ...
  val tex_data() const {
    return val(typed_memory_view(m->ntexdata * 1, m->tex_data));
  }
  val mat_texid() const {
  ...
  val eq_active() const {
    return val(typed_memory_view(m->neq * 1, m->eq_active0));
  }
  val eq_solref() const {
...
public:
  ...
  void solveM2(val x, val y, int n) {
    return mj_solveM2(_model->ptr(), _state->ptr(),
                      reinterpret_cast<mjtNum *>(x["byteOffset"].as<int>()),
                      reinterpret_cast<mjtNum *>(y["byteOffset"].as<int>()),
                      reinterpret_cast<mjtNum *>(y["byteOffset"].as<int>()), n);
  }
  ...
  void _error(std::string msg) { return mju_error("%s", msg.c_str()); }
  ...
  void _warning(std::string msg) { return mju_warning("%s", msg.c_str()); }
...
EMSCRIPTEN_BINDINGS(mujoco_wasm) {
  ...
  // enum_<mjfCollision>("mjtCollision")
  //     .value("mjCOL_ALL", mjfCollision ::mjCOL_ALL)
  //     .value("mjCOL_PAIR", mjfCollision ::mjCOL_PAIR)
  //     .value("mjCOL_DYNAMIC", mjfCollision ::mjCOL_DYNAMIC);
  ...
  value_object<mjContact>("mjContact")
      .field(
          "dist",
          &mjContact::dist) // distance between nearest points; neg: penetration
      .field(
          "pos",
          &mjContact::pos) // position of contact point: midpoint between geoms
      .field("frame", &mjContact::frame) // normal is in [0-2]
      .field(
          "includemargin",
          &mjContact::includemargin) // include if dist<includemargin=margin-gap
      ...
      .field("mu", &mjContact::mu) // friction of regularized cone, set by
                                   // mj_makeConstraint
      ...
      .field("exclude", &mjContact::exclude) // 0: include, 1: in gap, 2: fused,
                                             // 3: equality, 4: no dofs
      .field("efc_address",
             &mjContact::efc_address); // address in efc; -1: not included,
                                       // -2-i: distance constraint i
  ...
  value_object<mjOption>("mjOption")
      ...
      .field("mpr_tolerance", &mjOption::ccd_tolerance) // MPR solver tolerance
      ...
      .field(
          "mpr_iterations",
          &mjOption::ccd_iterations) // maximum number of MPR solver iterations
      .field(
          "disableflags",
